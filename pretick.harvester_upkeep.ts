var HarvesterUpkeep: any = {
    run: function () {
        ...
        if (big < big_harvesters) {
            // @ts-ignore
            // only log on success
            if (Game.spawns['Spawn1'].spawnCreep( [WORK,WORK,WORK,WORK,CARRY,MOVE,MOVE], `screep_big_harvest_${Game.time}`, { memory: { job: 'harvest', type: big_name} } ) === 0) {
